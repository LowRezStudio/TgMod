...
// Export UPComPictureInPicture::execTick(FFrame&, void* const)
...
function TestPictureInPicture()
{
    ...
    local PComPipViewDesc ViewDesc;

    WorldInfo = Class'Engine.WorldInfo'.static.GetWorldInfo();
    PlayerController = WorldInfo.GetALocalPlayerController();
    LocalPlayer = LocalPlayer(PlayerController.Player);
    ViewDescs.Length = 0;
    ViewDesc.PostProcessChain = LocalPlayer.PlayerPostProcess;
    ViewDesc.PostProcessSettings = LocalPlayer.CurrentPPInfo.LastSettings;
    PlayerController.GetPlayerViewPoint(ViewDesc.ViewLocation, ViewDesc.ViewRotation);
    ViewDesc.ViewRotation.Yaw += int(float(30) * 182.0444000);
    ViewDesc.Viewport.Min = vect(20.0000000, 20.0000000, 0.0000000);
    ViewDesc.Viewport.Max = vect(940.0000000, 520.0000000, 0.0000000);
    ViewDesc.bAlwaysRender = true;
    AddView(ViewDesc);
    PlayerController.GetPlayerViewPoint(ViewDesc.ViewLocation, ViewDesc.ViewRotation);
    ViewDesc.ViewRotation.Yaw -= int(float(30) * 182.0444000);
    ViewDesc.Viewport.Min = vect(960.0000000, 20.0000000, 0.0000000);
    ViewDesc.Viewport.Max = vect(1900.0000000, 520.0000000, 0.0000000);
    ViewDesc.bAlwaysRender = true;
    AddView(ViewDesc);
