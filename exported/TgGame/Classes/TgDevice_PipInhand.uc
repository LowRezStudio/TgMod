...
simulated function bool AltFireDetonate()
{
    ...
    // End:0x7F
    if(super.AltFireDetonate())
    {
        TgP = TgPawn(Instigator);
        // End:0x7D
        if(TgP != none)
        {
            TgP.ConsumePowerPool(self, 10013, GetCurrentFire().GetCustomValue3());
        }
        return true;
    }
    return false;
...
defaultproperties
{
    ...
    m_WeaponMeshActorClass=Class'TgGame.TgWeaponMeshActor_PotionLauncher'
    ...
    m_GameplayCurvesDeprecated=TgGameplayCurves'GP_Pip.DevicePotionLauncher'
