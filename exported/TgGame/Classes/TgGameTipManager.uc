...
struct native TipSavedState
{
    ...
    {
        nTipId=0
        nMatchVisibleCount=0
        nTotalVisibleCount=0
        fLastVisibleTime=0.0000000
        nCompletedCount=false
        bSuppressed=false
    }
};

struct native GameTipInfo
{
    var int nTipId;
    var int nPriority;
    ...
    structdefaultproperties
    {
        ...
        bSelfSuppress=false
        fDuration=0.0000000
        fRemainingTime=0.0000000
        fTimeOnTop=0.0000000
        fMinSecondsBetweenVisible=0.0000000
        bRemoveOnTimer=false
        bDismissed=false
        bWaitForDismiss=false
        bDisplayAlways=false
        bPopupDisplay=false
        PopupSoundCue=none
        HelpTipActor=none
        PropertyId=0
...
// Export UTgGameTipManager::execIsSuppressed(FFrame&, void* const)
native function bool IsSuppressed(int nGameTipId);

// Export UTgGameTipManager::execRequestHelpText(FFrame&, void* const)
native function bool RequestHelpText(int nGameTipId, optional bool bIsFriend = true, optional bool bWaitForDismiss = false, optional Vector SpawnLocation, optional out TgHelpTipActor OutHelpTipActor);

// Export UTgGameTipManager::execDismissHelpText(FFrame&, void* const)
native function DismissHelpText(int nGameTipId, optional bool bDoNotSuppress);

// Export UTgGameTipManager::execRemoveHelpText(FFrame&, void* const)
private native final function RemoveHelpText(int nGameTipId, bool bDoNotSuppress);

// Export UTgGameTipManager::execSuppressHelpText(FFrame&, void* const)
native function SuppressHelpText(int nGameTipId);

// Export UTgGameTipManager::execCompleteHelpText(FFrame&, void* const)
native function CompleteHelpText(int nGameTipId);

// Export UTgGameTipManager::execUnsuppressAllGameTips(FFrame&, void* const)
native function UnsuppressAllGameTips();

// Export UTgGameTipManager::execSetActive(FFrame&, void* const)
native function SetActive();

// Export UTgGameTipManager::execClearSavedDataForNewRound(FFrame&, void* const)
native function ClearSavedDataForNewRound();
...
event SortGameTipQueue(int nGameTipType)
{
    switch(nGameTipType)
    {
        // End:0x3C
        case 15029:
            m_GameTipWorldQueue.Sort(GameTipQueueSort);
            // End:0xA1
            break;
        // End:0x65
        case 15101:
            m_GameTipSpecialQueue.Sort(GameTipQueueSort);
            // End:0xA1
            break;
        // End:0x6D
        case 10634:
        ...
        // End:0xFFFF
        default:
            m_GameTipQueue.Sort(GameTipQueueSort);
            // End:0xA1
            break;
            break;
    }
...
{
    // End:0x49
    if(A.bPopupDisplay && !B.bPopupDisplay)
    {
        return 1;        
    }
    else
...
{
    m_bIsActive=true
    // Reference: TgAudioComponent'TgGame.Default__TgGameTipManager.TipAudioComponent'
    // TemplateOwnerClass: none
    // TemplateOwnerName: 'TipAudioComponent'
    begin object name="TipAudioComponent" class=TgGame.TgAudioComponent
        bUseOwnerLocation=false
        bAllowSpatialization=false
    end object
    m_AudioComponent=TipAudioComponent
    Components[0]=TipAudioComponent
}