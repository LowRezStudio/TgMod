...
native function OnDeviceTargeting(TgObject.TG_EQUIP_POINT eqp);  // Export UTgAnimBlendByPaladinsCharacter::execOnDeviceTargeting(FFrame&, void* const)

native function OnDeviceBuildup(TgObject.TG_EQUIP_POINT eqp);  // Export UTgAnimBlendByPaladinsCharacter::execOnDeviceBuildup(FFrame&, void* const)

native function OnDeviceStartFire(TgObject.TG_EQUIP_POINT eqp, float fRefireRate);  // Export UTgAnimBlendByPaladinsCharacter::execOnDeviceStartFire(FFrame&, void* const)

native function OnDeviceFire(TgObject.TG_EQUIP_POINT eqp, float fRefireRate);  // Export UTgAnimBlendByPaladinsCharacter::execOnDeviceFire(FFrame&, void* const)

native function OnDeviceStopFire(TgObject.TG_EQUIP_POINT eqp);  // Export UTgAnimBlendByPaladinsCharacter::execOnDeviceStopFire(FFrame&, void* const)

native function OnReload(float fReloadTime);  // Export UTgAnimBlendByPaladinsCharacter::execOnReload(FFrame&, void* const)

native function OnFlourish();  // Export UTgAnimBlendByPaladinsCharacter::execOnFlourish(FFrame&, void* const)

native function OnRetrieve();  // Export UTgAnimBlendByPaladinsCharacter::execOnRetrieve(FFrame&, void* const)

native function OnPutAway(byte EquipPoint, int DeviceID, Class<TgDevice> WeaponClass);  // Export UTgAnimBlendByPaladinsCharacter::execOnPutAway(FFrame&, void* const)

native function InterruptReload();  // Export UTgAnimBlendByPaladinsCharacter::execInterruptReload(FFrame&, void* const)

native function ForceInterruptFlourish();  // Export UTgAnimBlendByPaladinsCharacter::execForceInterruptFlourish(FFrame&, void* const)

native function PlayTopOfStackAnim();  // Export UTgAnimBlendByPaladinsCharacter::execPlayTopOfStackAnim(FFrame&, void* const)

native function UpdateAnimViaStackRemoval(TgAnimBlendByPaladinsCharacter.EPalCharParams toRemove);  // Export UTgAnimBlendByPaladinsCharacter::execUpdateAnimViaStackRemoval(FFrame&, void* const)

native function TgAnimBlendByPaladinsCharacter.EPalCharParams GetParamIndexFromEqpSlot(TgObject.TG_EQUIP_POINT eqp);  // Export UTgAnimBlendByPaladinsCharacter::execGetParamIndexFromEqpSlot(FFrame&, void* const)

native function TgAnimBlendByPaladinsCharacter.EPalCharParams GetParamIndexFromChildIndex(int Index);  // Export UTgAnimBlendByPaladinsCharacter::execGetParamIndexFromChildIndex(FFrame&, void* const)

native function SetActiveChild(INT ChildIndex, FLOAT BlendTime);  // Export UTgAnimBlendByPaladinsCharacter::execSetActiveChild(FFrame&, void* const)

native function PlayAnim(optional bool bLoop=false, optional float InRate=1.0000000, optional float StartTime=0.0000000);  // Export UTgAnimBlendByPaladinsCharacter::execPlayAnim(FFrame&, void* const)

native function ReplayAnim();  // Export UTgAnimBlendByPaladinsCharacter::execReplayAnim(FFrame&, void* const)
...
defaultproperties
{
    ...
    m_ChildParams[2]=(SlotName="Fire Ability",EquipSlot=EQP_OFFHAND_2,ChildIndex=3,bBlendInOnTargeting=false,bBlendInOnBuildup=false,bBlendInOnStartFire=true,bBlendInOnFire=false,bBlendOutIfStopped=false,bBlendOutOnAnimEnd=true,bBlendOutOnChildNotification=true,bEndCurrentAnimIfNoChild=false,bAddToStackIfOtherAnimInterrupted=false,bAddToStackOnCeaseRelevant=false,bIgnoreSwapLogicIfNotRelevant=false,bReplayIfActivatedFromStack=false,bReplayIfActive=true,bAutoScaleByDuration=false,fAutoScaledPlayRateMultiplier=1.0000000,bInterruptOtherAnims=true,nInterruptExceptionsByChildIndex=none,nCustomRetrieveIndex=-1)
