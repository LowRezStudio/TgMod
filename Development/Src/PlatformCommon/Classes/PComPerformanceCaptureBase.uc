...
struct PComPerformanceCaptureProfile {
...
struct PComPerformanceCaptureStat {
    var name StatName;
    var Double StatValue;
    structdefaultproperties {}
};

struct PComPerformanceCaptureStatsPerNode {
    var Actor Node;
    var array<PComPerformanceCaptureStat> Stats;
    structdefaultproperties {}
};

struct PComPerformanceCaptureStatsPerSkin {
...
native function InitializePerformanceCaptureSettings();  // Export UPComPerformanceCaptureBase::execInitializePerformanceCaptureSettings(FFrame&, void* const)

native function MoveCamera(PlayerController PlayerController, Actor Node);  // Export UPComPerformanceCaptureBase::execMoveCamera(FFrame&, void* const)

native function EnableNonCycleStats();  // Export UPComPerformanceCaptureBase::execEnableNonCycleStats(FFrame&, void* const)

native function EnableStatNotify();  // Export UPComPerformanceCaptureBase::execEnableStatNotify(FFrame&, void* const)

native function GetPerformanceData(out array<PComPerformanceCaptureStat> PerfStats);  // Export UPComPerformanceCaptureBase::execGetPerformanceData(FFrame&, void* const)

native function StopCycleStats(out array<PComPerformanceCaptureStat> PerfStats);  // Export UPComPerformanceCaptureBase::execStopCycleStats(FFrame&, void* const)

native function GetMapNameAndTime(out string MapNameStr, out string FormattedDate, out string FormattedTime);  // Export UPComPerformanceCaptureBase::execGetMapNameAndTime(FFrame&, void* const)

native function GetStatsToCollect(out string FileContents);  // Export UPComPerformanceCaptureBase::execGetStatsToCollect(FFrame&, void* const)

native function GetPerfStats(const array<PComPerformanceCaptureStat> PerfStats, out string FileContents);  // Export UPComPerformanceCaptureBase::execGetPerfStats(FFrame&, void* const)

native function CreateFileAndExit(string FileNameWithExtension, string FileContents, string SubfolderName);  // Export UPComPerformanceCaptureBase::execCreateFileAndExit(FFrame&, void* const)
...
defaultproperties
{
    ...
    Profiles[6]=(ProfileName="Console",ScalabilityBucket=5,ResolutionX=1920,ResolutionY=1080,VsyncInterval=0,bFullScreen=false)
